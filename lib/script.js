 ...
 */
angular.module("mypro", []).controller("content", function ($scope, $http) {
    ...
    $scope.adduser = function () {
        ...
        if($scope.temp.length > 0) {
            while (j < $scope.temp.length) {
             ...
             while (i < $scope.temp.length) {
                 $scope.add_users.push({id: $scope.temp[i].id, name: $scope.temp[i].name, age: $scope.temp[i].age, email: $scope.temp[i].email});
                 $scope.temp.shift();
             }
